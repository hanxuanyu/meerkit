...
扩展只在用户主动连接并收到 `welcome` 后启用自动恢复。此后意外断线会按 1、2、5、10、30 秒退避重试，并通过 Chrome Alarm 覆盖后台 Service Worker 被回收的情况；首次连接失败、浏览器启动或用户主动断开不会主动尝试。连接地址、令牌或节点名称变化时会保留当前连接意图并立即重建连接。
