...
func New(frontend fs.FS, load ConfigLoader) *cobra.Command {
	command := &cobra.Command{Use: "serve", Short: "Start the Meerkit service", Args: cobra.NoArgs, RunE: func(command *cobra.Command, _ []string) error { return Run(command, frontend, load) }}
	BindFlags(command)
	return command
}
...
func Run(command *cobra.Command, frontend fs.FS, load ConfigLoader) error {
	...
	defer cancel()
	return application.RunServer(ctx, config, frontend)
}
