...
func TestTokenCopyConfiguration(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(path, []byte("security:\n  allow_token_copy: true\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	config, err := LoadConfigWithOptions(ConfigOptions{ConfigFile: path})
	if err != nil {
	...
	if !config.Security.AllowTokenCopy {
		t.Fatalf("token copy setting was not loaded: %#v", config.Security)
	}
}
