...
export function MonitorSummary({ monitor, descriptor }) {
  ...
  return <section className="monitor-summary"><div className="monitor-summary-heading"><div><span className="eyebrow">MONITOR CONFIGURATION</span><h2>监控内容</h2></div><Badge variant="outline">{monitor?.module_type?.toUpperCase()}</Badge></div><div className="monitor-summary-badges">{parameters.map((parameter) => <SummaryField key={parameter.key} parameter={parameter} value={values[parameter.key]} values={values} />)}</div></section>;
}
