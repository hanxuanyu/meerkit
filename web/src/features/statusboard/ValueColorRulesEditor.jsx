...
export function ValueColorRulesEditor({ type, source, onChange }) {
  ...
  return <div className="value-color-editor">
    <div className="value-color-heading"><div><strong>{type === "number" ? "精确值覆盖" : "特定值颜色"}</strong><span>{type === "number" ? "命中后优先于数值区间" : "未命中的文本使用默认颜色"}</span></div><IconButton className="section-add" size="sm" title="添加特定值颜色" aria-label="添加特定值颜色" onClick={addMapping}><Plus size={14} /></IconButton></div>
