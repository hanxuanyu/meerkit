...
export function ValueColorRulesEditor({ type, source, onChange }) {
  ...
  const addMapping = () => updateSource({ value_mappings: [...mappings, { value: type === "number" ? nextNumericValue(mappings) : `值${mappings.length + 1}`, level: "failure", label: "失败", color: "red" }] });
  ...
  return <div className="value-color-editor">
    <div className="value-color-heading"><div><strong>{type === "number" ? "精确值覆盖" : "特定值颜色"}</strong><span>{type === "number" ? "命中后优先于数值区间" : "未命中的文本使用默认颜色"}</span></div><IconButton variant="outline" size="sm" title="添加特定值颜色" aria-label="添加特定值颜色" onClick={addMapping}><Plus size={14} /></IconButton></div>
    ...
    <div className="value-color-list">{mappings.map((mapping, index) => { const preset = statusColorPreset(mapping.color, mapping.level); return <div className="value-color-row" key={index}><Input type={type === "number" ? "number" : "text"} step={type === "number" ? "any" : undefined} value={mapping.value} onChange={(event) => updateMapping(index, { value: event.target.value })} placeholder={type === "number" ? "精确数值" : "精确文本（可为空）"} aria-label={`第 ${index + 1} 条匹配值`} /><label className="threshold-abnormal-toggle"><Checkbox checked={mapping.level !== "success"} onCheckedChange={(checked) => updateMapping(index, { level: checked ? "failure" : "success", label: checked ? "异常" : "正常" })} /><span>异常</span></label><PresetColorPicker color={preset.id} level={mapping.level} ariaLabel={`第 ${index + 1} 条匹配颜色`} onChange={(selected) => updateMapping(index, { color: selected.id })} /><IconButton size="sm" title="删除特定值规则" aria-label="删除特定值规则" onClick={() => removeMapping(index)}><Trash2 size={13} /></IconButton></div>; })}</div>
  </div>;
}
