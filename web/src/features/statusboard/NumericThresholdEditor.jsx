...
export function NumericThresholdEditor({ thresholds, onChange }) {
  ...
  return <div className="numeric-threshold-editor">
    ...
    <div className="threshold-interval-list">{thresholds.map((threshold, index) => <div className="threshold-interval-row" key={index}><strong>{intervalLabel(thresholds, index)}</strong><label className="threshold-abnormal-toggle"><Checkbox checked={threshold.level !== "success"} onCheckedChange={(checked) => updateThreshold(index, { level: checked ? "failure" : "success", label: checked ? "异常" : "正常" })} /><span>异常</span></label><PresetColorPicker color={threshold.color} level={threshold.level} disabledColorIDs={thresholds.filter((_, itemIndex) => itemIndex !== index).map((item) => statusColorPreset(item.color, item.level).id)} ariaLabel={`第 ${index + 1} 个区间颜色`} onChange={(preset) => updateThreshold(index, { color: preset.id })} />{index < thresholds.length - 1 ? <IconButton className="repeat-row-remove" size="sm" title="删除区间分界" aria-label="删除区间分界" onClick={() => removeBoundary(index)}><Trash2 size={13} /></IconButton> : <span className="threshold-row-spacer" />}</div>)}</div>
  </div>;
}
