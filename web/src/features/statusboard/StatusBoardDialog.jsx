...
function valueStyle(valueType) {
  if (valueType === "text") return { value_mappings: [{ value: "", match_type: "exact", level: "failure", label: "失败", color: "red" }], default_level: "success", default_label: "正常", default_color: "green" };
...
export function StatusBoardDialog({ item, monitors, channels, onClose, onSaved, onError }) {
  ...
  const isDuplicate = Boolean(item?.__duplicate);
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      await api(isEditing ? `/api/v1/status-board/items/${item.id}` : "/api/v1/status-board/items", { method: isEditing ? "PATCH" : "POST", body: JSON.stringify(payload) });
      onSaved();
    } catch (error) {
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="modal-wide status-board-dialog"><DialogHeader><div><span className="eyebrow">{isDuplicate ? "DUPLICATE BOARD ITEM" : "STATUS BOARD ITEM"}</span><DialogTitle>{isEditing ? "编辑看板项" : isDuplicate ? "复制看板项" : "添加看板项"}</DialogTitle><DialogDescription>{isDuplicate ? "基于现有配置创建新的看板项，可在保存前调整差异。" : "选择一次执行中的状态或结果值，并按执行次数观察变化。"}</DialogDescription></div></DialogHeader><form onSubmit={submit}><div className="modal-body">
    <div className="form-grid"><label className="field"><Label>名称</Label><Input required value={name} onChange={(event) => setName(event.target.value)} placeholder="例如：API 响应耗时" /></label><label className="field"><Label>监控项</Label><Select value={monitorID} onValueChange={changeMonitor} disabled={isEditing}><SelectTrigger><SelectValue placeholder="选择监控项" /></SelectTrigger><SelectContent className="status-board-select-content">{monitors.map((monitor) => <SelectItem key={monitor.id} value={monitor.id}>{monitor.name}</SelectItem>)}</SelectContent></Select></label></div>
    ...
    {effectiveType === "text" && <div className="form-section"><div className="form-section-title"><div><h3>文本颜色</h3><p>可按精确文本或正则表达式指定状态，其余内容使用默认状态。</p></div></div><ValueColorRulesEditor type="text" source={source} onChange={setSource} /></div>}
    <div className="form-section"><div className="form-section-title"><div><h3>趋势通知</h3><p>规则进入异常和恢复正常时各发送一次。</p></div><IconButton className="section-add" size="sm" title="添加趋势规则" aria-label="添加趋势规则" onClick={addTrendRule}><Plus size={14} /></IconButton></div><div className="trend-rule-list">{trendRules.length ? trendRules.map((rule, index) => <TrendRuleRow key={rule.id} rule={rule} index={index} numeric={effectiveType === "number"} onChange={(patch) => updateTrendRule(index, patch)} onRemove={() => removeTrendRule(index)} />) : <div className="condition-empty">点击右上角 + 添加趋势规则</div>}</div></div>
  ...
  </div><DialogFooter><Button type="button" variant="ghost" onClick={onClose}>取消</Button><Button type="submit" disabled={saving || loadingSources || !monitorID}>{saving ? "保存中..." : <><Check size={15} />{isDuplicate ? "创建副本" : "保存看板项"}</>}</Button></DialogFooter></form></DialogContent></Dialog>;
}
