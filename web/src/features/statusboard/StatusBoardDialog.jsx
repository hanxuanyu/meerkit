...
const levels = { success: "正常", warning: "警告", failure: "失败" };
...
const defaultThresholds = [{ maximum: null, level: "success", label: "正常" }];
...
export function StatusBoardDialog({ item, monitors, channels, onClose, onSaved, onError }) {
  ...
  const selectResult = (selection) => {
    ...
    setSource({ kind: "result_field", result_set, field, path: "", value_type });
    setThresholds(value_type === "number" ? defaultThresholds : []);
    setInvert(false);
    setTrendRules([]);
  };
  const changeValueType = (value_type) => {
    setSource((current) => ({ ...current, value_type }));
    setThresholds(value_type === "number" ? defaultThresholds : []);
    setTrendRules((current) => value_type === "number" ? current : current.filter((rule) => rule.type === "consecutive" || rule.type === "count"));
  };
  const addThreshold = () => setThresholds((current) => [...current.slice(0, -1), { maximum: 0, level: "warning", label: "警告" }, current.at(-1) || defaultThresholds[0]]);
  const updateThreshold = (index, patch) => setThresholds((current) => current.map((threshold, itemIndex) => itemIndex === index ? { ...threshold, ...patch } : threshold));
  const removeThreshold = (index) => setThresholds((current) => current.filter((_, itemIndex) => itemIndex !== index));
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="modal-wide status-board-dialog"><DialogHeader><div><span className="eyebrow">STATUS BOARD ITEM</span><DialogTitle>{item ? "编辑看板项" : "添加看板项"}</DialogTitle><DialogDescription>选择一次执行中的状态或结果值，并按执行次数观察变化。</DialogDescription></div></DialogHeader><form onSubmit={submit}><div className="modal-body">
    ...
    {effectiveType === "number" && <div className="form-section"><div className="form-section-title"><div><h3>数值区间</h3><p>按递增上界匹配颜色，最后一项覆盖其余数值。</p></div><IconButton size="sm" title="添加数值区间" aria-label="添加数值区间" onClick={addThreshold}><Plus size={14} /></IconButton></div><div className="status-threshold-list">{thresholds.map((threshold, index) => <div className="status-threshold-row" key={index}><span className={`status-color-swatch level-${threshold.level}`} />{index < thresholds.length - 1 ? <Input type="number" step="any" value={threshold.maximum ?? ""} onChange={(event) => updateThreshold(index, { maximum: Number(event.target.value) })} aria-label="区间上界" /> : <span className="status-unbounded">其余数值</span>}<Select value={threshold.level} onValueChange={(level) => updateThreshold(index, { level })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(levels).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select><Input value={threshold.label} onChange={(event) => updateThreshold(index, { label: event.target.value })} aria-label="区间标签" />{thresholds.length > 1 && <IconButton size="sm" title="删除数值区间" aria-label="删除数值区间" onClick={() => removeThreshold(index)}><Trash2 size={14} /></IconButton>}</div>)}</div></div>}
