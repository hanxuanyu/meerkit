...
import { PublicStatusBoardPage } from "../../pages/PublicStatusBoardPage";

export function AuthGate() {
  const match = window.location.pathname.match(/^\/shared\/status-board\/([^/]+)\/?$/);
  if (match) return <PublicStatusBoardPage token={decodePublicToken(match[1])} />;
  return <AdministratorAuthGate />;
}

function decodePublicToken(value) {
  try { return decodeURIComponent(value); } catch { return value; }
}

function AdministratorAuthGate() {
...
function AuthForm({ mode, busy, error, onSubmit }) {
  ...
  return <main className="auth-screen"><section className="auth-panel">
    ...
    <form onSubmit={(event) => { event.preventDefault(); void onSubmit(setup ? { access_key: accessKey, confirm } : { access_key: accessKey }); }}>
      <label><Label>访问密钥</Label><PasswordInput autoFocus minLength={12} required value={accessKey} onChange={(event) => setAccessKey(event.target.value)} /></label>
      {setup && <label><Label>确认访问密钥</Label><PasswordInput minLength={12} required value={confirm} onChange={(event) => setConfirm(event.target.value)} /></label>}
