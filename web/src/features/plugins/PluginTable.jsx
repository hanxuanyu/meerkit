...
export function PluginTable({ plugins, busy, onDetails, onLogs, onToggleEnabled, onUninstall }) {
  return <Table className="plugin-table">
...
function PluginRow({ plugin, busy, onDetails, onLogs, onToggleEnabled, onUninstall }) {
  ...
  return <TableRow>
    ...
    <TableCell className="plugin-modules-cell" data-label="模块"><div className="plugin-modules">{(plugin.modules || []).map((module) => <Badge key={module.type} variant="outline">{module.name || module.type}</Badge>)}</div></TableCell>
