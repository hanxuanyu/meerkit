...
export function PluginDetailDialog({ plugin, loading, trusting, onClose, onTrust }) {
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}>
    <DialogContent className="modal-wide plugin-detail-dialog">
      <DialogHeader><div><span className="eyebrow">PLUGIN DETAIL</span><DialogTitle>{plugin.name}</DialogTitle><DialogDescription>{plugin.desp || "暂无插件描述"}</DialogDescription></div></DialogHeader>
      <div className="modal-body plugin-detail-body">
        <TrustNotice plugin={plugin} trusting={trusting} onTrust={onTrust} />
        <section className="plugin-detail-meta" aria-label="插件元数据">
          <Meta label="插件 ID" value={plugin.id} mono />
          <Meta label="版本" value={plugin.version} mono />
          <Meta label="开发者" value={plugin.vendor} />
          <Meta label="协议范围" value={protocol.min === undefined ? "-" : `${protocol.min} - ${protocol.max}`} mono />
          <Meta label="包文件" value={plugin.package_name} mono />
          <Meta label="包 SHA-256" value={plugin.package_sha256} mono wide />
          {plugin.signer_key_id && <Meta label="签名 Key ID" value={plugin.signer_key_id} mono />}
          {plugin.signer_fingerprint && <Meta label="签名指纹" value={plugin.signer_fingerprint} mono wide />}
        </section>
...
function Meta({ label, value, mono = false, wide = false }) {
  return <div className={wide ? "is-wide" : ""}><span>{label}</span><strong className={mono ? "is-mono" : ""}>{value || "-"}</strong></div>;
}
