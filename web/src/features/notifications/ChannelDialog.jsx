...
export function ChannelDialog({ channel, notifiers, monitors, modules, onClose, onSaved, onError, onTest }) {
  ...
  const submit = async (event) => {
    ...
    setSaving(true);
    try {
      await api(channel ? `/api/v1/notification-channels/${channel.id}` : "/api/v1/notification-channels", { method: channel ? "PATCH" : "POST", body: JSON.stringify({ name, notifier_type: type, enabled: channel?.enabled ?? true, config: sanitizeValues(parameters, config) }) });
      onSaved();
    } catch (error) {
