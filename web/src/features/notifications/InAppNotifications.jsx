...
export function NotificationCenterPage({ refreshVersion = 0, initialNotificationID = "", onNotificationRouteChange, onOpenExecution, onMarkRead, onMarkAllRead, onNotificationsDeleted, unreadCount = 0, browserNotificationStatus = "disabled", onToggleBrowserNotifications }) {
  ...
  return <div className="page-stack">
    <PageHeader eyebrow="IN-APP NOTIFICATIONS" title="通知中心" description="查看监控触发、恢复和系统测试产生的站内通知。" />
    <Card className="section-card notification-center-card">
      <div className="toolbar list-toolbar notification-center-toolbar"><div className="notification-center-search list-toolbar-search"><Search size={15} /><Input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="搜索标题或通知内容" aria-label="搜索站内通知" /><Select value={status} onValueChange={(value) => { setStatus(value); setPageInfo((current) => ({ ...current, page: 1 })); }}><SelectTrigger className="notification-status-select" aria-label="按已读状态筛选"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">全部通知</SelectItem><SelectItem value="unread">仅未读</SelectItem></SelectContent></Select></div><div className="notification-center-toolbar-actions list-toolbar-actions"><div className="browser-notification-heading-control" title={browserNotificationLabel(browserNotificationStatus)}>{browserNotificationStatus === "enabled" ? <BellRing size={15} /> : <BellOff size={15} />}<span>浏览器通知</span><Switch checked={browserNotificationStatus === "enabled"} aria-label={browserNotificationStatus === "enabled" ? "关闭浏览器通知" : "开启浏览器通知"} onCheckedChange={onToggleBrowserNotifications} /></div><IconButton variant="outline" size="default" title="刷新" aria-label="刷新通知" onClick={() => void load()}><RefreshCw size={15} /></IconButton><Button size="sm" variant="outline" className="destructive-outline" disabled={loading} onClick={() => setConfirmDeleteRead(true)}><Trash2 size={14} />删除已读</Button><Button size="sm" variant="outline" disabled={!unreadCount} onClick={markAll}><CheckCheck size={14} />全部标记已读</Button></div></div>
