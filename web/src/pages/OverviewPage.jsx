...
export function OverviewPage({ monitors, modules = [], loading, onCreate, onOpen, onRun, onViewRecords }) {
  const stats = { total: monitors.length, active: monitors.filter((item) => item.enabled && item.module_available !== false).length, alert: monitors.filter((item) => item.runtime_state?.condition_active && item.module_available !== false).length, healthy: monitors.filter((item) => item.runtime_state?.last_success && item.module_available !== false).length };
