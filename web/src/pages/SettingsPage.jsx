    ...
    <Dialog open={open} onOpenChange={(nextOpen) => nextOpen ? onOpenChange(true) : requestClose()}>
      <DialogContent className="admin-key-dialog">
        ...
        <form onSubmit={submit}>
          <div className="modal-body admin-key-dialog-body"><label className="admin-key-field"><span>当前密钥</span><Input type="password" autoFocus autoComplete="current-password" required value={currentAccessKey} onChange={(event) => setCurrentAccessKey(event.target.value)} /></label><label className="admin-key-field"><span>新密钥</span><Input type="password" autoComplete="new-password" minLength={12} required value={accessKey} onChange={(event) => setAccessKey(event.target.value)} /></label><label className="admin-key-field"><span>确认新密钥</span><Input type="password" autoComplete="new-password" minLength={12} required value={confirm} onChange={(event) => setConfirm(event.target.value)} /></label>{mismatch && <p className="admin-key-form-error">两次输入的新密钥不一致</p>}</div>
