...
export function NotificationsPage({ channels = [], onCreate, onRefresh, onEdit, onDuplicate, onToggleEnabled, togglingChannelId }) {
  ...
  return <div className="page-stack">
    ...
    <Card className="section-card">
      <div className="toolbar list-toolbar channel-toolbar"><div className="channel-search list-toolbar-search"><Search size={15} /><Input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="搜索渠道名称或类型" aria-label="搜索通知渠道" /></div><div className="list-toolbar-actions"><IconButton variant="outline" size="default" title="刷新" aria-label="刷新" onClick={onRefresh}><RefreshCw size={15} /></IconButton><Button size="sm" onClick={onCreate}><Plus size={15} />添加渠道</Button></div></div>
      {hasMatches ? <>
