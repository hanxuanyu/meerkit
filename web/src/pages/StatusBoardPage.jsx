...
export function StatusBoardPage({ monitors, channels, refreshVersion = 0, onOpenExecution, notify }) {
  ...
  return <div className="page-stack status-board-page"><PageHeader eyebrow="STATUS BOARD" title="状态看板" description={`${snapshot.groups.length} 个监控分组 · ${itemCount} 个看板项`} /><div className="status-board-toolbar"><span><Activity size={15} />{snapshot.groups.length} 个分组 · {itemCount} 个看板项</span><div><Button size="sm" variant="outline" onClick={() => setShareDialogOpen(true)} disabled={!snapshot.groups.length}><Share2 size={14} />分享</Button><Button size="sm" onClick={() => setEditing(null)} disabled={!monitors.length}><Plus size={14} />添加看板项</Button></div></div>
