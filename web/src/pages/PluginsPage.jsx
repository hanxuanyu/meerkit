...
export function PluginsPage({ notify }) {
  ...
  return <div className="page-stack">
    <PageHeader className="page-heading-with-action" eyebrow="EXTENSIONS" title="监控插件" description="管理当前平台可用的监控类型与插件进程。" actions={<div className="page-heading-actions"><input ref={fileInput} hidden type="file" accept=".zip,.tar.gz,application/zip,application/gzip" onChange={upload} /><Button onClick={() => fileInput.current?.click()} disabled={busy === "upload"}>{busy === "upload" ? <LoaderCircle className="spin" size={15} /> : <Upload size={15} />}导入插件</Button><IconButton variant="outline" size="default" title="扫描插件目录" aria-label="扫描插件目录" disabled={Boolean(busy)} onClick={() => { void perform("scan", () => api("/api/v1/plugins/scan", { method: "POST" }), "插件目录扫描完成"); }}><FolderSync size={15} /></IconButton></div>} />
    <Card className="section-card plugin-card">
      <div className="toolbar monitor-toolbar plugin-toolbar"><div className="monitor-search"><Search size={15} /><Input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="搜索名称、ID、开发者或模块" aria-label="搜索插件" /></div><div className="monitor-filters"><Select value={status} onValueChange={updateFilter(setStatus)}><SelectTrigger className="monitor-filter-select" aria-label="按运行状态筛选"><SelectValue placeholder="全部状态" /></SelectTrigger><SelectContent><SelectItem value="all">全部状态</SelectItem><SelectItem value="enabled">已启用</SelectItem><SelectItem value="disabled">已停用</SelectItem><SelectItem value="healthy">运行中</SelectItem><SelectItem value="degraded">异常</SelectItem></SelectContent></Select><Select value={trustState} onValueChange={updateFilter(setTrustState)}><SelectTrigger className="monitor-filter-select" aria-label="按信任状态筛选"><SelectValue placeholder="全部信任状态" /></SelectTrigger><SelectContent><SelectItem value="all">全部信任</SelectItem><SelectItem value="development">开发源码</SelectItem><SelectItem value="official">官方可信</SelectItem><SelectItem value="trusted">已验证</SelectItem><SelectItem value="untrusted">待信任</SelectItem><SelectItem value="unsigned">未签名</SelectItem></SelectContent></Select><IconButton variant="outline" size="default" title="刷新" aria-label="刷新" disabled={loading} onClick={() => { void load(); }}><RefreshCw className={loading ? "spin" : ""} size={15} /></IconButton></div></div>
